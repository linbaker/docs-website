...
Method tracers are software probes you can put on a method of any class. The probes use alias method chaining to insert themselves when the target methods execute and gather custom instrumentation on their performance. Method tracers must occur within a transaction in order to be reported to New Relic.
